...
StructuredBuffer<TransformationMatrix> gInstanceTransformationMatrices : register(t4);
...
float4 main(VertexShaderInput input, uint instanceId : SV_InstanceID) : SV_POSITION
{
    ...
    TransformationMatrix transformationMatrix = gInstanceTransformationMatrices[instanceId];
    const float4 worldPosition = mul(skinnedPosition, transformationMatrix.World);
