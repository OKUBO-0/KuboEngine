...
void GameplayDebugContext::InitializeCamera()
{
...
void GameplayDebugContext::FinalizeCamera()
{
	Engine::CameraSystem::CameraManager::GetInstance()->RemoveCamera(
		"directxgame_debug");
}

void GameplayDebugContext::UpdateCamera()
{
	camera_.SetTranslate(cameraPosition_);
	camera_.SetRotate(cameraRotation_);
	camera_.SetFarClip(500.0f);
	...
	cameraManager->SyncCamera("directxgame_debug", &camera_);
	if (cameraEnabled_) {
		cameraManager->SetActiveCamera("directxgame_debug");
	}
}

void GameplayDebugContext::Load(
	Player* player,
...
void GameplayDebugContext::Save(
	const Player* player,
